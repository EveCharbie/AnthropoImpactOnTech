...
for name in athletes_number.keys():
    ...
    arms_twist_potential = abs(excel[index_arms, 4])
    index_hips = np.where(excel[:, 0] == name + " bras en  bas, jambes tilt")[0][0]
    hips_twist_potential = abs(excel[index_hips, 4])
...
axs[0].set_xlim(0.45, 0.8)
axs[0].set_ylim(3.8, 10.15)
axs[1].set_xlim(0.45, 0.8)
axs[1].set_ylim(5.9, 10.6)
axs[2].set_xlim(0.27, 0.42)
...
for i in range(6):
    ...
    axs[i, 0].set_xlim(0.45, 0.8)
    axs[i, 0].set_ylim(3.8, 10.15)
    axs[i, 1].set_xlim(0.45, 0.8)
    axs[i, 1].set_ylim(5.9, 10.6)
    axs[i, 2].set_xlim(0.27, 0.42)
