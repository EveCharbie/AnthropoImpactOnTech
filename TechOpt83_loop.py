...
stunts = [[3,"vrille_et_demi"],[5,"double_vrille_et_demi"], [7, "triple_vrille_et_demi"]]
#r_stunts =[3, 5, 7] # number of rotations for twist
athletes = ["Sarah", "Benjamin", "AlAd", "ZoTs", "AdCh", "WeEm", "SoMe", "OlGa", "MaJa", "MaCu", "LaDe", "KaMi", "KaFu", "FeBl", "EvZl", "ElMe", "AuJo"]
for athlete in athletes :
    for stunt in stunts :
        r_stunt = stunt[0]
        stunt_name = stunt[1]
        folder = "/home/mickaelbegon/Documents/Stage_Lisa/AnthropoImpactOnTech/Nouvelles_solutions/Solutions_" + stunt_name
        ...
        def prepare_ocp(
                biorbd_model_path: str, n_shooting: int, final_time: float, n_threads: int,
            ...
            x_bounds[3].min[Zrot, FIN] = r_stunt * 3.14 - .1  # complete la vrille
            x_bounds[3].max[Zrot, FIN] = r_stunt * 3.14 + .1
            ...
            # la vrille autour de z
            x_bounds[4].min[Zrot, :] = r_stunt * 3.14 - .1  # complete la vrille
            x_bounds[4].max[Zrot, :] = r_stunt * 3.14 + .1
        ...
        def main():
            ...
            biorbd_model_path = "/home/mickaelbegon/Documents/Stage_Lisa/AnthropoImpactOnTech/Models/" + f'{path}'
            Mod = Model(model=biorbd_model_path, savesol= True)
            n_shooting = (40, 100, 100, 100, 40)

            ocp = prepare_ocp(Mod.model, n_shooting=n_shooting, n_threads=25, final_time=1.87)
            ocp.add_plot_penalty(CostType.ALL)
            if Mod.print_ocp :
                ocp.print(to_graph=False)
        ...
        if __name__ == "__main__":
                ...
                #graphs_solutions.path
                #for states in :
              #  fig.savefig(f'{folder}/{athlete}')

        # fig.suptitle(suptitre)
        # fig.tight_layout()
        # fig.savefig(f'Solutions/{stunt_name}/{athlete}/{suptitre}.pdf')
