...
def random_mass(mass):
    sigma = mass * 0.0075
    mu = mass
    mass_random = random.gauss(mu, sigma)  # erreur on veut 0.75% de la masse totale
    return mass_random
...
def random_com(com):
    ...
    for i in range(len(com)):
        sigma = np.array(com[i] * 0.005) # erreur on veut 0.5% de la taille totale
        mu = np.array(com[i])
        com_random[i] = random.gauss(mu, sigma)
    return com_random.tolist()


def random_inertia(inertia):
    inertia_random = np.array(inertia)
    for i in range(len(inertia)):
        row_inertia = inertia_random[i]
        for j in range(len(row_inertia)):
            col_inertia = row_inertia[j]
            sigma = col_inertia * 0.01
            mu = col_inertia
            inertia_random[i][j] = random.gauss(mu, sigma)

    return inertia_random
...
class Pelvis(BioModSegment):
    def __init__(
        ...
        com = O
        mass = human.P.mass
        mass = random_mass(mass)
        inertia = human.P.rel_inertia
        inertia = random_inertia(inertia)
...
class Thorax(BioModSegment):
    def __init__(
        ...
        inertia_global = random_inertia(inertia_global)
...
def main():
    models_path = "/home/lim/Documents/Stage_Lisa/AnthropoImpactOnTech/Models/Models_random"

    for filename in os.listdir(models_path):
        if os.path.isfile(filename):
            if filename.endswith(".txt"):
                print(filename)
                for random_indice in [j for j in range(30)]:
                    ...
                    random_models_path = "/home/lim/Documents/Stage_Lisa/AnthropoImpactOnTech/Models/Models_random"
                    athlete = filename.removesuffix(".txt")
                    new_filename = random_models_path + "/Random/" + athlete + f"_random_{random_indice}.bioMod"
                    with open(new_filename, "w") as f:
                        bio = biohuman.__str__()
                        f.writelines(bio)
