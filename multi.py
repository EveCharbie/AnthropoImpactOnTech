...
def set_x_bounds(biorbd_model, fancy_names_index, final_time):
   ...
   # x_bounds[0].max[fancy_names_index["XrotC"], FIN] = -2.35 #min du model
    ...
    borne_inf = ( x_bounds[0].min[fancy_names_index["vX"]:fancy_names_index["vZ" ] +1, DEBUT] + np.cross(r, x_bounds[0].min
                                                                                                        [fancy_names_index
                                                                                                            ["vXrot"]
                                                                                                        :fancy_names_index
                                                                                                            ["vZrot" ] +1, DEBUT]) )[0]
    borne_sup = ( x_bounds[0].max[fancy_names_index["vX"]:fancy_names_index["vZ" ] +1, DEBUT] + np.cross(r, x_bounds[0].max
                                                                                                        [fancy_names_index
                                                                                                            ["vXrot"]
                                                                                                        :fancy_names_index
                                                                                                            ["vZrot" ] +1, DEBUT]) )[0]
    x_bounds[0].min[fancy_names_index["vX"]:fancy_names_index["vZ" ] +1, DEBUT] = min(borne_sup[0], borne_inf[0]), min \
        (borne_sup[1], borne_inf[1]), min(borne_sup[2], borne_inf[2])
    x_bounds[0].max[fancy_names_index["vX"]:fancy_names_index["vZ" ] +1, DEBUT] = max(borne_sup[0], borne_inf[0]), max \
...
def prepare_ocp(
    ...
    # JeCh
    objective_functions.add(ObjectiveFcn.Mayer.SUPERIMPOSE_MARKERS, node=Node.END, first_marker='MidMainG',
                            second_marker='CibleMainG', weight=1000, phase=5)
    objective_functions.add(ObjectiveFcn.Mayer.SUPERIMPOSE_MARKERS, node=Node.END, first_marker='MidMainD',
                            second_marker='CibleMainD', weight=1000, phase=5)
    ...
    les_coudes = [fancy_names_index["ZrotABD"], fancy_names_index["XrotABD"],
                  fancy_names_index["ZrotABG"], fancy_names_index["XrotABG"]]
    ## AuJo
    objective_functions.add(minimize_dofs, custom_type=ObjectiveFcn.Lagrange, node=Node.ALL_SHOOTING, dofs=les_coudes,
                            targets=np.zeros(len(les_coudes)), weight=10000, phase=0)
    objective_functions.add(minimize_dofs, custom_type=ObjectiveFcn.Lagrange, node=Node.ALL_SHOOTING, dofs=les_bras,
                            targets=np.zeros(len(les_bras)), weight=10000, phase=2)
    objective_functions.add(minimize_dofs, custom_type=ObjectiveFcn.Lagrange, node=Node.ALL_SHOOTING, dofs=les_bras,
                            targets=np.zeros(len(les_bras)), weight=10000, phase=3)
    objective_functions.add(minimize_dofs, custom_type=ObjectiveFcn.Lagrange, node=Node.ALL_SHOOTING, dofs=les_coudes,
                            targets=np.zeros(len(les_coudes)), weight=10000, phase=4)
    ##JeCh
    objective_functions.add(minimize_dofs, custom_type=ObjectiveFcn.Lagrange, node=Node.ALL_SHOOTING, dofs=les_coudes,
                            targets=np.zeros(len(les_coudes)), weight=10000, phase=5)
    objective_functions.add(minimize_dofs, custom_type=ObjectiveFcn.Lagrange, node=Node.ALL_SHOOTING, dofs=les_bras,
                            targets=np.zeros(len(les_bras)), weight=10000, phase=7)
    objective_functions.add(minimize_dofs, custom_type=ObjectiveFcn.Lagrange, node=Node.ALL_SHOOTING, dofs=les_bras,
                            targets=np.zeros(len(les_bras)), weight=10000, phase=8)
    objective_functions.add(minimize_dofs, custom_type=ObjectiveFcn.Lagrange, node=Node.ALL_SHOOTING, dofs=les_coudes,
                            targets=np.zeros(len(les_coudes)), weight=10000, phase=9)
    # argparse
    ...
    ## AuJo
    objective_functions.add(minimize_dofs, custom_type=ObjectiveFcn.Mayer, node=Node.END,
                            dofs=[fancy_names_index["XrotC"]], targets=[0, 0], weight=10000, phase=3)
    ## JeCh
    objective_functions.add(minimize_dofs, custom_type=ObjectiveFcn.Mayer, node=Node.END,
                            dofs=[fancy_names_index["XrotC"]], targets=[0, 0], weight=10000, phase=8)
...
def main():
    ...
    if save_sol_FLAG:  # switch manuelle
        ...
        np.save(f"Solutions_multiModel_constraints/{nom}-{str(n_shooting).replace(', ', '_')}-{temps}-t.npy", sol.phase_time)
