...
for athlete in folder_per_athlete:
    ...
    for filename in os.listdir(results_path_this_time):
        ...
        if filename.removesuffix(".pkl")[-3] == "C":
            ...
            if file_name not in done_athlete:
                ...
                ocp = prepare_ocp(
                    biorbd_model_path=model, nb_twist=int(key), seed=int(filename.split("_")[-2]), n_threads=1
                )
                if os.path.isfile(f):
                    if filename.endswith(".pkl"):
                        with open(filename, "rb") as f:
                            ...
                            # integrated
                            integrated_sol = ocp.solve()
                            data_integrated.append(integrated_sol["sol"].integrate(merge_phases=True))
    ...
    cmap_1 = cm.get_cmap("viridis")
    cmap_2 = cm.get_cmap("magma")
    ...
    for i in range(len(Bruit)):
        ...
        cost = COST[i]
        q = Data[i]
        q_integrated = data_integrated[i]
        ...
        if FLAG_SAME_FIG:
            ...
            for i in range(len(q[0])):
                ...
                for phase in range(len(q)):
                    Q += q[phase][i].tolist()[:]
                    Q_integrated += q_integrated[phase][i].tolist()[:]
                if i == 0:
                    axs[i].plot(Q, color=cmap_1(ratio), label=bruit)
                    axs[i].plot(Q_integrated, color=cmap_2(ratio), label=bruit)

                else:
                    im = axs[i].plot(Q, color=cmap_1(ratio))
                    im = axs[i].plot(Q, color=cmap_2(ratio))
    ...
    if FLAG_SAME_FIG:
        ...
        plt.subplots_adjust(left=0.05, right=0.8, hspace=0.4)
        plt.savefig(f"{folder_graphs}/{file_name}", dpi=300)
        plt.show()
